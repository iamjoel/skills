...
- `功能 Review` 至少包含一个详情单元，并按独立重构职责、迁移边界或行为不变量重复；不要把不相关重构合并为一个单元。
...
- 类型：<没有完成重构目标 | 引入了新的问题 | 代码质量问题 | 性能问题>
- 具体说明：<结合真实调用或消费路径和代码说明重构、回归、代码质量或性能风险>
...
- 建议修复：<给出可执行的迁移补全、结构调整、回归消除或性能优化方向>
