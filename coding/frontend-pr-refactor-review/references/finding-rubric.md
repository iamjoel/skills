...
本 rubric 改编自 OpenAI Codex review rubric。把它作为 finding 的统一准入层；Refactor 检查和 Refactor Map 由主 skill 与 `review-focus.md` 控制。

## Candidate 流程

1. 沿每个重构单元和不变量收集 candidate findings。
...
5. 把合格问题挂到对应 `rN` 或 `xN`，再分配稳定 ID、priority 和 confidence。
...
前端 finding 还必须定位到 changed frontend lines 或其直接消费的前端契约，并说明被破坏的不变量、触发条件、错误结果和用户影响。修复方向必须落在本次 PR 的前端范围内。
...
- 同一根因影响多个重构单元且需要统一修复时使用 `xN.iN`。
- 完成所有重构单元和跨单元项的 coverage pass，不要只返回第一个问题。
...
- `0.90–1.00`：changed line、真实调用链、旧/新行为和错误结果均有直接证据。
- `0.75–0.89`：证据充分，但缺少运行验证或仍有一个受控的不确定因素。
- `< 0.75`：通常归入 `Not verified` 或明确问题。

## Correctness 与 Verdict

- `Frontend correctness`：补丁是否保持声明的不变量且没有引入前端 bug。
...
- 解释 changed logic、被破坏的不变量、错误结果、用户影响和紧凑修复方向。
